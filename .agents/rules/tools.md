...
Before writing any query here, read `project/supabase/README.md` — every tool
...
- Every write tool (`add_project_row`, `create_project_field`,
  ...
  `.agents/skills/verification-envelope/SKILL.md` and perform its own
...
- Idempotency: every write tool call must compute and check the
  idempotency key described in
  `.agents/skills/idempotency-key/SKILL.md` before inserting.
