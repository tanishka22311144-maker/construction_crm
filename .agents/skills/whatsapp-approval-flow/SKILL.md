...
The node must save checkpoint state and let the function exit — never
block waiting inside the invocation. A later webhook (the approver's
reply) resumes the same `thread_id` with `Command(resume=...)`.
