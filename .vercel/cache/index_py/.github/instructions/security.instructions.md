---
description: 'Authorization, approval, and identity handling rules'
applyTo: 'services/authorization.py,services/approvals.py,services/verification.py,services/database.py'
---
...
- `service_role` Supabase clients are instantiated only inside
  `services/database.py`'s admin-transaction path, never in tool code,
  never in graph nodes directly.
